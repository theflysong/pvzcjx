...
std::map<int, bool> PotatoMineIsSproutOuted;
std::map<int, bool> ScaredyShroonIsScared;
...
void EventHandler::UpdatePlants()
{
	...
	for (int i = 0; i < plants_num; i++) {
		...
		if (plant->Type == PlantType::PotatoMine && plant->State == PlantState::POTATO_SPROUT_OUT && !PotatoMineIsSproutOuted[plant->Index]) {
			PotatoMineIsSproutOuted[plant->Index] = true;
			InvokeEvent(new EventPlantPotatoMineSproutOuted(plant), true);
		}
		if (plant->Type == PlantType::Scaredyshroon && plant->State == PlantState::SCAREDYSHROOM_SCARED && !ScaredyShroonIsScared[plant->Index]) {
			ScaredyShroonIsScared[plant->Index] = true;
	...
	for (int i = 0; i < plants_num; i++)
	{
		...
		if (ok)
		{
			...
			PotatoMineIsSproutOuted[plant->Index] = false;
			ScaredyShroonIsScared[plant->Index] = false;
		}
	}
